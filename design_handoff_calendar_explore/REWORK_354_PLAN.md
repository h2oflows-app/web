...
| **A2** | api | **Invite rework (PRIORITY)**: mig `000146` `DROP plan_members`→`CREATE run_invites` (F2), run-scoped `InviteToRun`/accept/dismiss/join/crew, `BuildRunInvite` single-VEVENT ICS, single-run email, run-page token carve-out re-keyed to `run_invites`, calendar crew-run inclusion (decision #8). Plus from A1/W1 review: `is_owner`/`my_role` on `GET /plan-runs/{id}` (W1 hid owner affordances without it); discover query gains `AND NOT paddled`. | L | A1 |
...
**A2+W4 are LOCKSTEP, not staggered (correction, post-implementation):** unlike A1/W1 (see §7's "degrades, doesn't crash" note, which does NOT apply here), deploying A2 to prod before pushing the combined W2+W4 web build is a hard outage, not a graceful degrade. `GET /me/invites` flips from the old `{invites:[{event, runs:[...]}]}` grouped shape to A2's flat `{invites:[{id, status, ..., run:{...}}]}` — every pre-W4 consumer (`useInvites.ts`'s `recomputeUnread()`, `InviteBanner.vue`, `InviteFeedCard.vue`) dereferences `invite.event.*` / `invite.runs.forEach(...)` unconditionally, which throws (not `undefined` silently) on an item that has no `event`/`runs` key at all. `NotificationBell` (inside `AppHeader`, which nearly every page mounts) calls `useInvites().refresh()` on mount for any authenticated user, so ANY user with at least one pending invite white-screens on effectively every page they visit post-A2/pre-W4 — `/calendar`, `/invites`, and the event page are the surfaces most likely to be hit first, but the blast radius is AppHeader-wide, not limited to those three. Deploy A2 and push this combined W2+W4 branch back-to-back with no gap; do not land A2 and stop.
...
- **Deploy-order skew (Netlify web auto-deploy vs manual api).** Always **deploy the A-wave before pushing the dependent W branch.** Old web build reading `data.plan` after key rename gets `undefined` — degrades (blank), doesn't crash, but order anyway. api tolerates stale request bodies (keep silently-ignoring `visibility` etc., matching existing `Create` pattern). **Correction (post-A2/W4 implementation): this "degrades, doesn't crash" claim is specific to the A1/W1 rename (an absent object key reads as `undefined` and blanks a template) and does NOT hold for A2/W4's invite re-key** — `GET /me/invites` moving from a grouped `{event, runs:[]}` shape to A2's flat `{run:{...}}` shape means a stale pre-W4 web build dereferences `invite.event.host_handle` / iterates `invite.runs` on an item with neither key present, which throws inside `useInvites.ts`'s `recomputeUnread()` (called from `NotificationBell`, mounted in `AppHeader` on nearly every page) rather than rendering blank — see §5's runbook addendum for the resulting blast radius and the LOCKSTEP requirement this implies for A2+W4.
