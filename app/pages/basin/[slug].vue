  ...
  <div class="min-h-screen bg-neutral-50 dark:bg-neutral-950">
    ...
    <main class="max-w-5xl mx-auto px-4 py-6 space-y-6">
      ...
      <section class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 divide-y divide-neutral-100 dark:divide-neutral-800 overflow-hidden">
        ...
        <div v-else-if="fetchDone && mapData.length === 0" class="px-4 py-6 text-center text-sm text-neutral-400">
...
// Slugs from dashboard gauge contexts — used as a hint to the API, but the
// API also scans all user_reaches by basin slug, so this need not be exhaustive.
...
// Fetch once on mount. Store is synchronously hydrated from localStorage before
// mount so reachSlugs is available immediately; immediate:true handles both SPA
// nav and hard refresh. Fetch always runs — even with no gauge context slugs the
// API will find user_reaches by basin slug for authenticated callers.
